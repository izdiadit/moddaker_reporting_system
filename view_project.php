						...
						<table class="table table-condensed m-0 table-hover">
							...
							<tbody>
								...
								if ($_SESSION['login_type'] != 1)
									$tasks = $conn->query("SELECT * FROM task_list where project_id = {$id}  AND  employee_id =  {$_SESSION['login_id']} order by date_created desc");
								if ($_SESSION['login_type'] == 1 || $_SESSION['login_type'] == 2)
									$tasks = $conn->query("SELECT * FROM task_list where project_id = {$id}   order by date_created desc");
								...
								while ($row = $tasks->fetch_assoc()) :
									...
									if (isset($_GET['tid'])) :
										if ($row['id'] == $_GET['tid']) :
									?>
											<tr id="notified" style="background: yellow;">
											<?php endif;
										...
										if (!isset($_GET['tid'])) :
											...
											<td class="text-center"><?php
																							$useridd = $row['employee_id'];
																							$username = $conn->query("SELECT concat(firstname,lastname) as name FROM users where id = $useridd ");
																							while ($row2 = $username->fetch_assoc()) :
																								echo $row2['name'];
																							endwhile
																							?></td>
											<td>
...
<script>
	...
	$(document).ready(function() {
		//location.reload();	
		var notifiedTask = document.getElementById('notified')
		var color =  notifiedTask.style.background
		if(color == 'yellow'){
			setTimeout(()=>{
				notifiedTask.style.background = 'aliceblue';
			},3000)
